class HuffPQ:
    ...
    def _heapify(self, index):
        ...
        smallest = index
        if (left_index < len(self._pq)) and (self._pq[left_index].compare(self._pq[index]) < 0):
            smallest = left_index
        if (right_index < len(self._pq)) and (self._pq[right_index].compare(self._pq[smallest]) < 0):
    ...
    def dequeue(self):
        ...
        root = self._pq[0]
        self._pq[0] = self._pq[len(self._pq) - 1]
        self._heapify(0)

        del self._pq[len(self._pq) - 1]
        return root
